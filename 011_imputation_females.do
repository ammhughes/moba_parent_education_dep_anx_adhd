*SEX-STRATIFIED IMPUTATION AND MAIN MODELS: FEMALES ONLY
...
*EXCLUSIONS:
*as for the other analysis, restrict to people with at least some questionnaire information.
count if nQ_present>0 & complete_trio==1
*40,608

//Restrict to observations with valid genetic data
*restrict to full trios:
...
save "scratch/mi_analysis_dataset_pre_females",replace
use  "scratch/mi_analysis_dataset_pre_females",clear

************************************************************************
*ACTUAL IMPUTATION: FOR RUNNING ON TSD
/*
...
/**ACTUAL IMPUTATION: FOR RUNNING ON COLOSSUS

*JUST THE INSIDE PART OF THE LOOP - BUT DOING 50 AS ONE BATCH
...
//Run imputation 
//Here I've used ologit for variables that have relatively few catagories. For variables with more than 4 catagories I've used truncreg. 
//Check distributions
...
, add(50) rseed(100) dots savetrace(impstats_females.dta) augment 

compress
save "scratch/impx50_mi_raw_dataset_females", replace
...
save "scratch/impx50_mi_analysis_dataset_females", replace
