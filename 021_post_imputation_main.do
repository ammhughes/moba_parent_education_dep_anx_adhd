...
count
*40,879 after fixing all issues
...
capture erase "output\tables\imputed_dep_anx_adhd_subscales.txt"
capture erase "output\tables\imputed_dep_anx_adhd_subscales.xls"
...
estout using "output\tables\imputed_dep_anx_adhd_subscales.xls", cells ("b(fmt(2)) ci_l(fmt(2)) ci_u(fmt(2)) se(fmt(3)) p(fmt(3))") keep(*eduyears* c_z_ea4_pgs) replace title(maternal and paternal education to age 8 outcomes) note("comparing non-genetic and within-family MR models.")
...
graph save output/graphs/imputed_`i'_coefeplot.gph, replace
}
*combine:
grc1leg2 output/graphs/imputed_smfq_coefeplot.gph output/graphs/imputed_scared_coefeplot.gph output/graphs/imputed_adhd_full_coefeplot.gph, cols(1) scheme(s1mono) imargin(10 10 0 0) position(6) 
graph save output/graphs/imputed_alloutcomes_coefeplot.gph, replace
graph export output/graphs/imputed_alloutcomes_coefeplot.tif, replace width(1200)
...
*do this via estout as before:
capture erase "output\tables\ln_dep_anx_adhd_subscales.txt"
capture erase "output\tables\ln_dep_anx_adhd_subscales.xls"
...
estout using "output\tables\ln_dep_anx_adhd_subscales.xls", cells ("b(fmt(2)) ci_l(fmt(2)) ci_u(fmt(2)) se(fmt(3)) p(fmt(3))") keep(*eduyears* c_z_ea4_pgs) replace title(maternal and paternal education to age 8 outcomes) note("comparing non-genetic and within-family MR models.")
...
keep (mother_eduyears father_eduyears c_z_ea4_pgs) drop(_cons) xline(0) grid(none) msize(small) title("{bf:$`i'_title}", size(medsmall) pos(4) ring(0) color(black)) scheme(s1mono) graphregion(margin(l=10 r=10)  color(white)) legend(rows(1) size(small)) xsc(r(-0.1 0.1)) xtick(-0.1 -0.08 -0.06 -0.04 -0.02 0.0 0.02 0.04 0.06 0.08 0.1) xlabel(-0.1 -0.08 -0.06 -0.04 -0.02 0.0 0.02 0.04 0.06 0.08 0.1) xlabel(,labsize(small)) ylabel(,labsize(small)) legend(region(lwidth(none)))
graph save output/graphs/imputed_ln_`i'_coefeplot.gph, replace
}
*combine:
grc1leg2 output/graphs/imputed_ln_smfq_coefeplot.gph output/graphs/imputed_ln_scared_coefeplot.gph output/graphs/imputed_ln_adhd_full_coefeplot.gph, cols(1) scheme(s1mono) imargin(10 10 0 0) position(6) 
graph save output/graphs/imputed_ln_alloutcomes_coefeplot.gph, replace
graph export output/graphs/imputed_ln_alloutcomes_coefeplot.tif, replace width(1200)